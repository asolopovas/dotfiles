...
if type "xrandr"; then
  ...
  if [ "$primary" == "primary" ]; then 
    if [ $display == 'eDP-1' ]; then
      CPU_TEMP=$cpu_temp	NETWORK_INTERFACE=$net_interface MONITOR=$display polybar --reload laptop > "/tmp/polybar-laptop.log" 2>&1 &
    else 
      echo "loading main bar...\n"
      CPU_TEMP=$cpu_temp	NETWORK_INTERFACE=$net_interface MONITOR=$display polybar --reload main > "/tmp/polybar-main.log" 2>&1 &
    fi
  fi
done
fi
