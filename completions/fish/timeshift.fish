...
complete -c timeshift -s t -l tags -d "Add tags to snapshot" -xa "O B H D W M"
...
complete -c timeshift -l snapshot-device -d "Specify backup device" -xa "(__timeshift_needs_arg)"

complete -c timeshift -f -l yes -d "Answer YES to all confirmation prompts"
complete -c timeshift -f -l btrfs -d "Switch to BTRFS mode"
complete -c timeshift -f -l rsync -d "Switch to RSYNC mode"
complete -c timeshift -f -l debug -d "Show additional debug messages"
complete -c timeshift -f -l verbose -d "Show rsync output"
