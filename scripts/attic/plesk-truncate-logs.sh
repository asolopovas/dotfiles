...
remove_file() {
    ...
    if [[ -f "$file" ]]; then
        ...
        size=$(stat -c%s "$file" 2>/dev/null || echo 0)
        if $DRY_RUN; then
            log "[dry-run] Would remove $file ($(numfmt --to=iec "$size"))"
        else
            rm -f "$file"
            log "Removed $file (freed $(numfmt --to=iec "$size"))"
        fi
        TOTAL_FREED=$((TOTAL_FREED + size))
        FILE_COUNT=$((FILE_COUNT + 1))
    fi
...
truncate_user_logs() {
    ...
    for domain_dir in "$vhosts_dir"/*/; do
        ...
        # Rotated/compressed log archives
        remove_glob "${logs_dir}/*.gz"

        # Processed log files
        truncate_glob "${logs_dir}/*.processed"
...
truncate_php_fpm_logs() {
    ...
    truncate_glob "/var/log/php-fpm/*.log"
    remove_glob "/var/log/php-fpm/*.gz"
}
...
Truncate active logs and remove rotated .gz archives across
Plesk server logs, PHP-FPM logs, and per-domain logs.
