...
backup_path() {
	...
	mv "$path" "$backup"
	print_color yellow "  backup: $path -> $backup"
}
...
setup_opencode() {
	...
	if [[ -d "$root_oc" ]]; then
		replace_path_with_symlink /opt/dotfiles/.config/opencode /opt/opencode-config
		print_color green "  /opt/opencode-config -> /opt/dotfiles/.config/opencode"
	else
...
setup_vhosts() {
	...
	while IFS=$'\t' read -r domain plesk_user home_dir; do
		...
		if [[ -d "$dotfiles_dir/.config/opencode" ]]; then
			replace_path_with_symlink "$dotfiles_dir/.config/opencode" "$home_dir/.config/opencode"
