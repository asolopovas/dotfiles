...
EOF
    ...
    sudo cp "$tmp/ca.crt" "$tmp/ca.pem" "$tmp/ca.key" "$ssl/"
    sudo chmod 600 "$ssl/squid-self-signed.key" "$ssl/ca.key"
    sudo chmod 644 "$ssl/squid-self-signed.crt" "$ssl/squid-self-signed.pem" "$ssl/squid-self-signed_dhparam.pem" "$ssl/ca.crt" "$ssl/ca.pem"
...
install_ca() {
    ...
    # Install system-wide CA certificate (Root CA, not server cert)
    sudo cp "$ssl/ca.pem" /usr/local/share/ca-certificates/squid-ca.crt
    sudo update-ca-certificates >/dev/null

    # Also install in system's ca-certificates bundle for better compatibility
    sudo mkdir -p /etc/ssl/certs
    sudo cp "$ssl/ca.pem" /etc/ssl/certs/squid-ca.pem
    sudo c_rehash /etc/ssl/certs >/dev/null 2>&1 || true
    ...
    [ -n "${SUDO_USER:-}" ] && {
        ...
        if [ -d "$user_home/.mozilla/firefox" ]; then
            for profile in "$user_home/.mozilla/firefox"/*default* "$user_home/.mozilla/firefox"/*.default*; do
                if [ -d "$profile" ]; then
                    ...
                    if sudo -u "$SUDO_USER" certutil -A -n "Squid Root CA" -t "TCu,Cu,Tu" -i "$ssl/ca.pem" -d "$profile" 2>/dev/null; then
        ...
        if [ -d "$user_home/snap/firefox/common/.mozilla/firefox" ]; then
            for profile in "$user_home/snap/firefox/common/.mozilla/firefox"/*default* "$user_home/snap/firefox/common/.mozilla/firefox"/*.default*; do
                if [ -d "$profile" ]; then
                    ...
                    # Remove existing certificate first to avoid duplicates
                    sudo -u "$SUDO_USER" certutil -D -n "Squid Root CA" -d "$profile" 2>/dev/null || true
                    if sudo -u "$SUDO_USER" certutil -A -n "Squid Root CA" -t "TCu,Cu,Tu" -i "$ssl/ca.pem" -d "$profile" 2>/dev/null; then
        ...
        if command -v certutil >/dev/null; then
            ...
            if sudo -u "$SUDO_USER" certutil -A -n "Squid Root CA" -t "TCu,Cu,Tu" -i "$ssl/ca.pem" -d sql:"$nssdb_dir" 2>/dev/null; then
    ...
    if openssl verify -CAfile /usr/local/share/ca-certificates/squid-ca.crt "$ssl/ca.pem" >/dev/null 2>&1; then
    ...
    if curl --cacert "$ssl/ca.pem" https://httpbin.org/get >/dev/null 2>&1; then
...
https_port $HTTPS_INTERCEPT_PORT intercept ssl-bump generate-host-certificates=on dynamic_cert_mem_cache_size=$SSL_CERT_CACHE_SIZE tls-cert=$ssl/ca.crt tls-key=$ssl/ca.key
...
setup_iptables() {
    ...
    if test_internet_connectivity; then
        ...
        error "❌ Transparent proxy setup failed - regular proxy still available on localhost:$PROXY_PORT"
    else
...
test_proxy_functionality() {
    ...
    if curl -s -L --proxy http://localhost:$PROXY_PORT --cacert "$ssl/ca.pem" \
...
main() {
    ...
    if curl -s --proxy http://localhost:$PROXY_PORT --cacert "$ssl/ca.pem" https://httpbin.org/get >/dev/null 2>&1; then
