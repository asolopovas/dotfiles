...
start_chrome() {
    ...
    sleep 1
    mkdir -p "$PROFILE_DIR"
    google-chrome-stable --remote-debugging-port=9222 --user-data-dir="$PROFILE_DIR" --no-first-run --no-default-browser-check >"$LOG_DIR/stdout.log" 2>"$LOG_DIR/stderr.log" &
