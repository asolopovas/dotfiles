...
DEFAULT_NAMES=(globals scripts init sync-ai inst-opencode sys-clean-vscode-server lint)
ALL_NAMES=(globals scripts init sync-ai inst-opencode inst-picom sys-clean-vscode-server lint)
...
declare -A SUITES=(
    ...
    ["sys-clean-vscode-server"]="$SCRIPT_DIR/test-sys-clean-vscode-server.bats"
    [lint]="$SCRIPT_DIR/test-lint.bats"
)

usage() {
    echo "Usage: $0 [--changed file ...|--list-changed file ...|globals|scripts|init|sync-ai|inst-opencode|inst-picom|sys-clean-vscode-server|lint ...|-f pattern]"
}
...
suite_deps() {
    ...
    case "$name" in
        globals)
            ...
            ;;
        scripts)
            printf '%s\n' tests/test-scripts.bats scripts/ops/ops-update-symlinks.sh scripts/inst/inst-php.sh scripts/plesk-codex-wrapper.sh scripts/plesk-codex-self-update.sh scripts/plesk-init.sh env/include-paths.sh env/env-vars.sh .profile helpers/ls-path helpers/fzf-code
            ;;
        init)
        ...
        inst-picom)
            ...
            ;;
        sys-clean-vscode-server)
            printf '%s\n' tests/test-sys-clean-vscode-server.bats scripts/sys/sys-clean-vscode-server
            ;;
        lint)
...
if [ "${1:-}" = "--list-changed" ]; then
    ...
    exec bats "$@" "$SCRIPT_DIR/test-globals.bats" "$SCRIPT_DIR/test-scripts.bats" "$SCRIPT_DIR/test-init.bats" "$SCRIPT_DIR/test-sync-ai.bats" "$SCRIPT_DIR/test-inst-opencode.bats" "$SCRIPT_DIR/test-sys-clean-vscode-server.bats" "$SCRIPT_DIR/test-lint.bats"
elif [ "$#" -eq 0 ]; then
